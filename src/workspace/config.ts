...
import { Row } from "../utils/data/Row";
...
/**
 * Configuration for an IndexedDB/Cache index.
 */
export interface TableIndex {
  /** The name of the index */
  name: string;
  /** The property path in the object to index */
...
/**
 * Configuration for a specific database table schema.
 */
export interface TableSchema<T extends string = TableName> {
  /** Unique name of the table */
  name: T;
  /** The primary key for the table (usually 'id') */
  keyPath: string;
  /** Optional list identifiers associated with this table */
  lists?: string[];
  /** Array of index definitions for searching/sorting */
  indexes: TableIndex[];
  /** * If true, data is persisted in a local-first manner. 
   * If false, might be transient or remote-only.
   */
  locally?: boolean;

  id_list: (_row: any) => string[];
}

export interface DBConfig<T extends string = TableName> {
  DB_NAME: string;
  DB_VERSION: number;
  TABLES: Record<T, TableSchema<T>>;
}

export const StockOS_CONFIG = {
  ...
  TABLES: <Record<TableName, TableSchema>>{
    ...
    end_customers: {
      ...
      locally: true,
      id_list: (_row: any) => ['end_customers'],
    },
    suppliers: {
      name: 'suppliers',
      keyPath: 'id',
    ...
    movements: {
      ...
      locally: false,
      id_list: (row: Row) => ['movements', row.product_id ? `movements_${row.product_id}` : null].filter(Boolean),
    ...
    purchase_orders: {
      ...
      locally: true,
      id_list: (row: Row) => ['purchase_orders', row.supplier_id ? `purchase_orders_${row.supplier_id}` : null,].filter(Boolean),
    },
    sales_orders: {
      name: 'sales_orders',
      keyPath: 'id',
      ...
      locally: true,
      id_list: (row: Row) => ['sales_orders', row.customer_id ? `sales_orders_${row.customer_id}` : null, row.customer_id ? `sales_orders_${row.customer_id}` : null,].filter(Boolean),
    },
    order_lines: {
      name: 'order_lines',
      keyPath: 'id',
      ...
      locally: false,
      id_list: (row: Row) => ['order_lines', row.order_id ? `items_${row.order_id}` : null,].filter(Boolean),
    },
    s_payments: {
      name: 's_payments',
      keyPath: 'id',
      ...
      locally: false,
      id_list: (row: Row) => ['payments', row.customer_id ? `payments_${row.customer_id}` : null, row.order_id ? `payments_${row.order_id}` : null,].filter(Boolean),
    },
    p_payments: {
      name: 'p_payments',
      keyPath: 'id',
      ...
      locally: false,
      id_list: (row: Row) => ['payments', row.supplier_id ? `payments_${row.supplier_id}` : null, row.order_id ? `payments_${row.order_id}` : null,].filter(Boolean),
...
export function getIdList<N extends string>(cfg: DBConfig<N>, tableName: N, row: Row) {
  const fn = cfg.TABLES[tableName].id_list;
  return fn ? fn(row) : [tableName];
}
export const dataEvents = new EventEmitter();

export const TABLE_NAMES: TableName[] = Object.keys(StockOS_CONFIG.TABLES) as TableName[];