...
function hlText(text: string, indices: number[]): string {
...
export function fuzzyMatch(text: unknown, query: string): MatchResult {
...
export function containsMatch(text: unknown, query: string): MatchResult {
  if (!query) return { match: true, hl: String(text) };
...
export function startsWithMatch(text: unknown, query: string): MatchResult {
  if (!query) return { match: true, hl: String(text) };
...
type SearchMode = 'fuzzy' | 'startswith' | 'contains';

export function applySearch(text: unknown, query: string, mode: SearchMode): MatchResult {
  if (!query) return { match: true, hl: String(text) };
