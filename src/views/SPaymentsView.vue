...
<template>
  <div>
    ...
    <div class="card p-0 overflow-hidden">
      <div class="tbl-wrap">
        <table>
          <thead><tr><th>Date</th><th>Customer</th><th>Order</th><th>Amount</th><th>Status</th><th>Notes</th><th>Actions</th></tr></thead>
          <tbody>
            ...
            <tr v-for="p in filtered" :key="p.id">
              ...
              <td>{{ db.getCustomer(p.customer_id)?.full_name || '—' }}</td>
              ...
              <td><div class="flex gap-1">
                ...
                <button class="btn btn-danger btn-sm" @click="async()=>{ if(await confirm('Delete payment?')){ try{await db.deleteSPayment(p.id);notify.success('Payment deleted')}catch(e){notify.error(e.message)} } }">Del</button>
              </div></td>
            </tr>
          </tbody>
        </table>
